...
if [ "${PLATFORM_NAME}" == "android" ]; then
  ...
  stf provider --allow-remote \
    --connect-url-pattern "${STF_PROVIDER_PUBLIC_IP}:<%= publicPort %>" \
    --storage-url ${PUBLIC_IP_PROTOCOL}://${STF_PROVIDER_PUBLIC_IP}:${PUBLIC_IP_PORT}/ \
    --screen-ws-url-pattern "${SOCKET_PROTOCOL}://${STF_PROVIDER_PUBLIC_IP}:${PUBLIC_IP_PORT}/d/${STF_PROVIDER_PUBLIC_IP}/<%= serial %>/<%= publicPort %>/"
...
#    --screen-ws-url-pattern "${SOCKET_PROTOCOL}://${STF_PROVIDER_PUBLIC_IP}:${PUBLIC_IP_PORT}/d/${STF_PROVIDER_PUBLIC_IP}/<%= serial %>/<%= publicPort %>/" \
  ...
  node /app/lib/cli ios-device --serial ${DEVICE_UDID} \
    ...
    --screen-ws-url-pattern "${SOCKET_PROTOCOL}://${STF_PROVIDER_PUBLIC_IP}:${PUBLIC_IP_PORT}/d/${STF_PROVIDER_PUBLIC_IP}/${DEVICE_UDID}/${STF_PROVIDER_MIN_PORT}/" \
